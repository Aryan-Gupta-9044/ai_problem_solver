...
if __name__ == "__main__":
    root = tk.Tk()
    # Optional: Add basic error handling if ttk themes aren't available
    try:
        app = CryptarithmeticApp(root)
    except tk.TclError:
        print("Warning: TTK themes might not be fully available. Using default look.")
        # Fallback to basic Tk if needed (would require removing ttk specifics)
        # For now, just let the error propagate if 'clam' theme fails severely
        app = CryptarithmeticApp(root) # Try again (might use default theme)
