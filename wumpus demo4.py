...
if __name__ == "__main__":
    ...
    root.deiconify()
    root.title("Wumpus World Adventure")
    root.attributes('-fullscreen', is_fullscreen.get())

    world = WumpusWorld(grid_size)
    world.agent_pos = game_positions['agent']
    world.set_wumpus(*game_positions['wumpus'])
    world.set_gold(*game_positions['gold'])
    for pit_pos in game_positions['pits']:
        world.set_pit(*pit_pos)

    app = WumpusWorldUI(root, world)
    root.mainloop()